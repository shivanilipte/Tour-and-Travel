...
import in.co.online.tourism.sys.bean.RoleBean;
import in.co.online.tourism.sys.bean.UserBean;
import in.co.online.tourism.sys.exception.ApplicationException;
import in.co.online.tourism.sys.model.RoleModel;
import in.co.online.tourism.sys.model.UserModel;
...
public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.forward(OTSView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(OTSView.WELCOME_CTL, request, response);
			return;
		}
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			try {
				...
				if (bean != null) {
					...
					if(bean.getRoleId()==2) {
					long pId=DataUtility.getLong(String.valueOf(session.getAttribute("pkId")));
					if(pId>0) {
						ServletUtility.redirect(OTSView.BOOKING_CTL+"?pId="+pId, request, response);
						return;
					}
					}
			...
			ServletUtility.redirect(OTSView.USER_REGISTRATION_CTL, request, response);
		return;
		}
		ServletUtility.forward(getView(), request, response);
